...
//            $gbd->beginTransaction();
            ...
            while ($row = pg_fetch_array($result, NULL, PGSQL_ASSOC)) {
                ...
                if (!empty($row["documento"])) {
                    ...
                    while ($rowAux = pg_fetch_array($resultContratos, NULL, PGSQL_ASSOC)) {
                        $sVer = "<button id=\"ver$i\" type=\"button\" class=\"btn btn-default btn-xs\" onclick=\"window.open('http://ingenieria.univalle.edu.co/facultadweb2/web/upload/files/" . $rowAux["archivo"] . "')\" formtarget=\"_blank\">VER</button>";
                    }
                }
                ...
                $tabla2 .= "<tr>";
                $tabla2 .= "<td style=\"max-width:20px\">" . $row["verifica"] . "</td>";
                $tabla2 .= "<td>";
                $tabla2 .= "<form class = \"form-horizontal\" id=\"form$i\" enctype=\"multipart/form-data\">
                               <div class=\"form-group form-control-sm\">
                                    <div class=\"row\">
                                        <div class=\"col-sm-6\">
                                            <input id=\"fechaexpedicion\" type=\"date\"  name=\"fechaexpedicion\" class=\"form-control input-sm\" value=\"" . date('Y-m-d') . "\" style=\"max-width:155px;min-width:15px\"/>
                                        </div>
                                        <div class=\"col-sm-6\">
                                           <div class=\"fileinput fileinput-new\" data-provides=\"fileinput\">
                                            <span class=\"btn btn-default btn-file btn-sm\"><span class=\"fileinput-new\">Selecionar</span>
                                                <span class=\"fileinput-exists input-sm\">Cambiar</span><input class=\"input-sm\" type=\"file\" name=\"archivo\" id=\"archivo\"/></span>
                ...
                $tabla2 .= "</td>";
                $tabla2 .= "<td>
                                <button id=\"subir$i\" class=\"btn btn-primary btn-xs\" type=\"button\" style=\"max-width:40px\">Subir</button>
                          </td>";
                $tabla2 .= "</tr>";
                
                $tabla2 .= "<script type=\"text/javascript\">
                                $(document).ready(function() {
                                    $('#subir$i').click(function() {
                                        $('#form$i').submit();
                                    });
                                });
                          </script>";
                                    ...
                                    $('#form$i').submit(function(event) {
                                        ...
                                        jQuery.ajax({
                                            ...
                                            success: function(response) {
                                                ...
                                                if(error === ''){
                                                    $('#informacion').html(\"Documento subido con &Eacute;xito\").show().fadeOut(5000);
                                                    $('#ver$i').attr(\"onClick\", \"window.open('http://ingenieria.univalle.edu.co/facultadweb2/web/upload/files/\"+ok+\"')\");
            ...
            $result = pg_query($gbd, "SELECT ID, NOMBRE FROM TERCEROS WHERE ID LIKE '$fktercero%'");

            while ($row = pg_fetch_array($result, NULL, PGSQL_ASSOC)) {
                $tabla1 .= "<div class=\"well\"><strong>NUMERO DOCUMENTO IDENTIDAD : </strong>" . $row["id"] . "<br/>";
                $tabla1 .= "<strong>NOMBRE COMPLETO : </strong>" . $row["nombre"] . "</div>";
                $tercero = $row["id"]. " - ".$row["nombre"];
                $tabla1 .= "<input id=\"tercero\" type=\"hidden\" value=\"$tercero\">";
            }
